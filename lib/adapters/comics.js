...
exports.addComic = async function(args) {
    if (args.length === 5) {
        const query = {
            text: `INSERT 
            INTO comicsArchive (publisher, volume, title, issue, copies, box)
            VALUES ($1, $2::int, $3, $4::int, $5::int, ${comicBox.box})
            ON CONFLICT 
            (title, volume, issue, publisher)
            DO UPDATE SET copies = comicsArchive.copies + $5::int`,
            values : args
        }
        ...
        const query = {
            text: `INSERT 
            INTO comicsArchive (publisher, volume, title, issue, box)
            VALUES ($1, $2::int, $3, $4::int, ${comicBox.box})
            ON CONFLICT 
            (title, volume, issue, publisher)
            DO UPDATE SET copies = comicsArchive.copies + 1`,
            values : args
        }
        await _addEntry(query);
    } else {
